...
// Archiviato: animazione orbitale con loghi tech, dots e forme geometriche
// usata in Hero.tsx fino al 2026-05-07. Sostituita da un effetto di sfumature
// animate. Conservata per eventuale riuso futuro.
...
export default function OrbitAnimation() {
  ...
  const ldotRefs  = useRef<(SVGLineElement  | null)[]>(Array(4).fill(null));
  const llogoRefs = useRef<(SVGLineElement  | null)[]>(Array(LOGOS.length).fill(null));
  const lpairRefs = useRef<(SVGLineElement  | null)[]>(Array(LOGO_PAIRS.length).fill(null));
  const chordRefs = useRef<(SVGLineElement  | null)[]>(Array(DOT_PAIRS.length).fill(null));
